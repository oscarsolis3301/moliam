...
function Hero({
  ...
  // typewriter
  const lines = React.useMemo(() => ['automation engineer', 'python · django · flask', 'ui/ux · ai · servicenow', 'builder. tinkerer. shipper.'], []);
  ...
  React.useEffect(() => {
    ...
    const onMove = e => {
      ...
      m.x = nx;
      m.y = ny;
      // Active only while the cursor is actually over the hero canvas bounds
    ...
    const render = () => {
      ...
      for (const n of nodes) {
        const {
          fx,
          fy
        } = flow(n.bx + n.px, n.by + n.py, t + n.seed * 17);
      ...
      for (let i = 0; i < nodes.length; i++) {
        ...
        for (const {
          ...
          ctx.lineWidth = 1 + nearMouse * 0.8;
          ctx.strokeStyle = `rgba(${r},${g},${bl},${alpha + nearMouse * 0.55})`;
          ctx.beginPath();
          ctx.moveTo(a.x, a.y);
          ctx.lineTo(b.x, b.y);
          ctx.stroke();
      ...
      for (const n of nodes) {
        ...
        ctx.fillStyle = `rgba(${r},${g},${bl},${baseA * pulseK + nearMouse * 0.55})`;
        ctx.beginPath();
        ctx.arc(n.x, n.y, n.r * pulseK + nearMouse * 2.6, 0, Math.PI * 2);
        ctx.fill();

        // soft halo for near-mouse nodes — stronger, layered
        if (nearMouse > 0.1) {
          // outer wide bloom
          ctx.fillStyle = `rgba(${accRGB[0]},${accRGB[1]},${accRGB[2]},${nearMouse * 0.12})`;
          ctx.beginPath();
          ctx.arc(n.x, n.y, n.r * 9 + nearMouse * 8, 0, Math.PI * 2);
          ctx.fill();
          // mid halo
          ctx.fillStyle = `rgba(${accRGB[0]},${accRGB[1]},${accRGB[2]},${nearMouse * 0.28})`;
          ctx.beginPath();
          ctx.arc(n.x, n.y, n.r * 5 + nearMouse * 5, 0, Math.PI * 2);
          ctx.fill();
        }
      }
      raf = requestAnimationFrame(render);
  ...
  return /*#__PURE__*/React.createElement("section", {
    ...
    className: "mono"
  }, "IRVINE, CA \xB7 AVAILABLE FOR WORK")), /*#__PURE__*/React.createElement("h1", {
    className: "hero-name"
  }, /*#__PURE__*/React.createElement("span", {
    className: "hero-line"
  }, "Oscar"), /*#__PURE__*/React.createElement("span", {
    className: "hero-line italic"
  }, "Solis.")), /*#__PURE__*/React.createElement("div", {
    className: "hero-sub"
  }, /*#__PURE__*/React.createElement("span", {
    className: "mono dim"
    ...
    onClick: () => onNav('work')
  }, /*#__PURE__*/React.createElement("span", null, "View work"), /*#__PURE__*/React.createElement("svg", {
    width: "14",
    height: "14",
    viewBox: "0 0 14 14"
    ...
    className: "mono tiny"
  }, "[ 01 / 05 ] \xB7 PORTFOLIO MMXXVI")), /*#__PURE__*/React.createElement("div", {
