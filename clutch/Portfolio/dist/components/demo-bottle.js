...
function BottleDemo({
  ...
  const [threeReady, setThreeReady] = React.useState(!!window.THREE);
  React.useEffect(() => {
    if (window.THREE) { setThreeReady(true); return; }
    if (typeof window.__loadThree === 'function') window.__loadThree();
    if (window.__threeReady && window.__threeReady.then) {
      window.__threeReady.then(() => setThreeReady(true));
    } else {
      const t = setInterval(() => {
        if (window.THREE) { setThreeReady(true); clearInterval(t); }
      }, 50);
  ...
  React.useEffect(() => {
    ...
    const baseRing = new THREE.Mesh(new THREE.TorusGeometry(0.56, 0.015, 16, 96), new THREE.MeshStandardMaterial({
      color: 0x0a0a0a,
      roughness: 0.8
    }));
    ...
    const capMain = new THREE.Mesh(new THREE.CylinderGeometry(0.26, 0.24, 0.18, 48), capMat);
    capMain.position.y = 0.04;
    capGroup.add(capMain);

    // cap top ridge
    const capTop = new THREE.Mesh(new THREE.CylinderGeometry(0.22, 0.26, 0.04, 48), capMat);
    ...
    for (let i = 0; i < 24; i++) {
      ...
      const ridge = new THREE.Mesh(new THREE.BoxGeometry(0.012, 0.14, 0.02), new THREE.MeshStandardMaterial({
        color: 0x0a0a0a,
        roughness: 0.9
      }));
    ...
    // carrying loop
    const loop = new THREE.Mesh(new THREE.TorusGeometry(0.08, 0.018, 12, 32, Math.PI), new THREE.MeshStandardMaterial({
      color: '#e8e2d4',
      roughness: 0.5
    }));
    ...
    capGroup.add(loop);
    const loopAnchor1 = new THREE.Mesh(new THREE.CylinderGeometry(0.022, 0.022, 0.03, 16), new THREE.MeshStandardMaterial({
      color: 0x111111
    }));
    ...
    const shadow = new THREE.Mesh(new THREE.PlaneGeometry(2.5, 1.2), new THREE.MeshBasicMaterial({
      map: shadowTex,
      transparent: true,
      depthWrite: false
    }));
  ...
  }, [theme, accent, bodyColor, capColor, threeReady]);
  React.useEffect(() => {
    stateRef.current.rotating = rotating;
  }, [rotating]);
  return /*#__PURE__*/React.createElement("div", {
    className: "demo-body bottle"
    ...
    className: "swatches"
  }, bodyColors.map(c => /*#__PURE__*/React.createElement("button", {
    key: c.hex,
    className: `sw ${bodyColor === c.hex ? 'on' : ''}`,
    style: {
      background: c.hex
    },
  ...
  }, rotating ? 'PAUSE' : 'SPIN'), /*#__PURE__*/React.createElement("span", {
    className: "mono tiny dim"
  ...
  }, "INSULATED \xB7 750ML \xB7 ANODIZED STEEL \xB7 PRESS + DRAG"));
}
