...
export async function onRequestGet(context) {
  try {
    ...
    if (!db) {
      return jsonResp(503, { success: false, error: "Database service unavailable." }, request);
         }
...
const token = tokenVal ? cookieMatch[1] : (url.searchParams.get('token') || null);

if (!token) {
  return jsonResp(401, { success: false, error: "Authentication token required." }, request);
}

// --- Session validation with parameterized query ---
const session = await db.prepare(
"SELECT u.id, u.email, u.name, u.role, u.company FROM sessions s JOIN users u ON s.token=? AND u.is_active = 1 AND s.expires_at > datetime('now')").bind(token).first();

    if (!session) {
      return jsonResp(401, { success: false, error: "Session invalid or expired." }, request);
          }
         ...
         if (action === 'leads') {
            ...
            if (isAdmin) {
              ...
              return jsonResp(200, { success: true, data: {
                ...
                console.error("Dashboard leads query error:", dbError.message);
                return jsonResp(500, { success: false, error: "Failed to retrieve leads data. Database error." }, request);
              }
             }
    ...
    if (action === 'pipeline') {
              ...
              return jsonResp(200, { success: true, data: {
                 ...
                 data: {
                  ...
                  fetchAt: new Date().toISOString()
                      ...
                      console.error("Dashboard pipeline query error:", dbError.message);
                      return jsonResp(500, { success: false, error: "Failed to retrieve pipeline data. Database error." }, request);
                  }
              }
      ...
      return jsonResp(200, { success: true, data: { id: session.id, name: session.name, email: session.email, role: session.role, company: session.company },
           errors: [],
              projects,
              updates,
              stats,
