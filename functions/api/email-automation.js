...
export async function onCron(event) { /** Check if database bound before attempting email sequence operations - prevents errors on unconfigured environments */ const db = event.env.MOLIAM_DB;
  ...
  try { const result = await db.prepare(
...
if (!submission || !submission.email) { await db.prepare("UPDATE email_sequences SET error_message=?, email_status='failed' WHERE id=?").bind('Missing submission record or email', row.id).run();
      ...
      if (!event.env.EMAIL_API_KEY) { console.log(`[MOCK] Would send to ${submission.email}`); await db.prepare(
               `UPDATE email_sequences SET email_status='skipped', sent_at=datetime('now') WHERE id=?`).bind(row.id).run(); continue; }
  ...
  return jsonResp(200, { success: true, queued_emails_sent: sentCount, total_attempted: (result.results?.length ?? 0) });} catch (err) {
