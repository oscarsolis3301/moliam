...
export async function onRequestPost(context) {
  ...
  try {
    ...
    await db.prepare(\
        \"CREATE TABLE IF NOT EXISTS clients(email TEXT PRIMARY KEY, password_hash TEXT NOT NULL, name TEXT DEFAULT '', role TEXT DEFAULT 'client', company TEXT)\"\n      ).run();
    await db.prepare(
    ...
    await db.prepare(
       "CREATE TABLE IF NOT EXISTS client_profiles(user_id INTEGER PRIMARY KEY, display_name TEXT, bio TEXT, avatar_url TEXT)"
     ).run();
    ...
    return new Response(JSON.stringify({
      ...
      users: [{ email: "admin@moliam.com", role: "admin" }]
    }), {
      status: 200,
