...
export async function onRequestPost(context) {
  ...
  try {
    ...
    // Insert Admin - use 10-column schema matching actual DB
    try {
      await db.prepare(
        "INSERT INTO users (email, password_hash, name, role, company, phone, avatar_url, is_active, created_at, last_login) VALUES (?, ?, ?, ?, ?, NULL, NULL, 1, datetime('now'), NULL)"
      ).bind("admin@moliam.com", adminHash, "Admin User", "superadmin", null).run();
    } catch (e) {
      // users table might not exist, skip
    }

    // Insert Oscar - same 10-column schema
    try {
      await db.prepare(
        "INSERT INTO users (email, password_hash, name, role, company, phone, avatar_url, is_active, created_at, last_login) VALUES (?, ?, ?, ?, ?, NULL, NULL, 1, datetime('now'), NULL)"
