...
export async function onRequestPost(context) {
  ...
  try {
    ...
    await db.prepare(
       "INSERT INTO users (name, email, password_hash, role, company, is_active, last_login, created_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?)"
     ).bind("Administrator", "admin@moliam.com", hash1, "admin", "Moliam", 1, new Date().toISOString(), new Date().toISOString()).run();

    await db.prepare(
       "INSERT INTO users (name, email, password_hash, role, company, is_active, last_login, created_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?)"
     ).bind("Oscar Solis", "oscar@onepluselectric.com", hash2, "user", "OnePlus Electric", 1, new Date().toISOString(), new Date().toISOString()).run();
    ...
    return new Response(JSON.stringify({ error: "Seed failed: " + err.message }), {
      status: 500,
      headers: { "Content-Type": "application/json" }
     });
   }
