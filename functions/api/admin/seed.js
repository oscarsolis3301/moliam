...
export async function onRequestPost(context) {
  ...
  try {
    ...
    await db.prepare(
      "INSERT INTO users (name, email, password_hash, role, company, is_active) VALUES (?, ?, ?, ?, ?, ?)"
    ).bind("Administrator", "admin@moliam.com", hash1, "admin", "Moliam", 1).run();

    await db.prepare(
      "INSERT INTO users (name, email, password_hash, role, company, is_active) VALUES (?, ?, ?, ?, ?, ?)"
    ).bind("Oscar Solis", "oscar@onepluselectric.com", hash2, "user", "OnePlus Electric", 1).run();
