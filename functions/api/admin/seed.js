...
export async function onRequestPost(context) {
  ...
  try {
    ...
    const oscarIdResult = await db
      .prepare(
        "SELECT id FROM users WHERE email='oscar@onepluselectric.com'",
      )
      .all();
    ...
    // Create test sessions for both users (3 params for 3 columns: user_id, token, created_at)
    await db
      .prepare(
        "INSERT INTO sessions(user_id, token, created_at) VALUES(?, ?, ?)",
      )
      .run(adminId, "test-session-1", "2026-04-14 10:00:00");

    await db
      .prepare(
        "INSERT INTO sessions(user_id, token, created_at) VALUES(?, ?, ?)",
      )
      .run(oscarId, "test-session-2", "2026-04-14 10:00:00");
