...
export async function onRequestPost(context) {
  ...
  try {
    ...
    await db.prepare(`DROP TABLE IF EXISTS users`).run();
     ...
     // Create users table 
    await db.prepare(`CREATE TABLE users (id INTEGER PRIMARY KEY, name TEXT NOT NULL, email TEXT UNIQUE NOT NULL, password_hash TEXT NOT NULL, role TEXT NOT NULL, enabled INTEGER DEFAULT 0)`).run();
  ...
  const saltedPassword1 = await hashPassword("Moliam2026!");
    ...
    await db.prepare(`INSERT INTO users (name, email, password_hash, role, enabled) VALUES (?, ?, ?, ?, ?)`).run("Administrator", "admin@moliam.com", saltedPassword1, "admin", 1);

      // Insert Oscar One Plus Electric user (ignore if exists)
    await db.prepare(`INSERT INTO users (name, email, password_hash, role, enabled) VALUES (?, ?, ?, ?, ?)`).run("Oscar Johnson", "oscar@onepluselectric.com", saltedPassword2, "user", 0);
