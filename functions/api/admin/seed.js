...
export async function onRequestPost(context) {
  ...
  try {
    ...
    // Clean existing tables - D1 does not support IF EXISTS
    ...
    // Insert into client_profiles with all 3 columns (excluding auto-incremented id)
  for (const u of users.results) {
    if (u && u.id) {
      await db.prepare("INSERT INTO client_profiles(display_name, user_id, bio) VALUES(?, ?, ?)")
         .run(["VisualArk", u.id, "AI-powered digital marketing agency"]);
     }
   }
