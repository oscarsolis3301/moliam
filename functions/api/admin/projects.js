...
async function requireAdmin(request, env) {
  ...
  if (!token) return jsonResp(401, { success: false, message: "Not authenticated." }, undefined, request);
   ...
   /** Secure database query with parameterized ? binding - no SQL injection possible here via .bind() method */
...
function jsonResp(status, body, request) {
  ...
  return new Response(JSON.stringify(body), { status, headers: {
         "Content-Type": "application/json","Access-Control-Allow-Origin": getAllowedOrigin(request),
     "Access-Control-Allow-Credentials": "true"
   }}); }
...
export async function onRequestGet(context) {
  ...
  try {
    ...
    return jsonResp(200, { success: true, projects }, request); } catch (err) {
...
export async function onRequestPost(context) {
  ...
  const setupFee = data.setup_fee || 0;
    ...
    if (!userId || !name) return jsonResp(400, { success: false, message: "Client ID and project name required." }, undefined, request);
   ...
   const validTypes = ["website", "gbp", "lsa", "retainer"];
       ...
       if (!validTypes.includes(type)) return jsonResp(400, { success: false, message: `Type must be one of: ${validTypes.join(", ")}` }, undefined, request);
          ...
          try { const client = await db.prepare("SELECT id FROM users WHERE id=? AND role='client'").bind(userId).first();


     if (!client) return jsonResp(404, { success: false, message: "Client not found." }, undefined, request);

             // Insert project record with auto-generated creation timestamp and notes field - uses ? binding for database safety
     ...
     return jsonResp(201, { success: true, message:`"${name}" created successfully.`, project: { id: projectId, name, type, monthly_rate: monthlyRate, setup_fee: setupFee } }, request);
              ...
              return jsonResp(500, { success: false, message: "Server error occurred during processing." }, undefined, request);
