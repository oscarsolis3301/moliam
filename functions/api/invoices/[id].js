...
export async function onRequestGet(context) {
  ...
  if (pathSegments.length < 2) {
    return jsonResp(400, { error: true, message: "Invoice ID required." }, request);
  }

  const id = parseInt(pathSegments[1]);
  if (!id || isNaN(id)) {
    return jsonResp(400, { error: true, message: "Invalid invoice ID." }, request);
  }
  ...
  try {
    ...
    if (!invoice) {
      return jsonResp(404, { error: true, message: "Invoice not found." }, request);
    }
  ...
  try {
    ...
    if (status) {
      if (!['draft', 'sent', 'paid', 'overdue'].includes(status)) {
        return jsonResp(400, { error: true, message: "Invalid status value." }, request);
      }
      query += `status = ?,`;
      params.push(status);
