...
export async function onRequestPost(context) {
  ...
  if (session.role !== 'admin' && session.role !== 'superadmin') return jsonResp(403, { success: false, error: true, message: "Admin required." }, request);
  ...
  try {
    ...
    // Validation
    if (!contact_id || !items) {
      return jsonResp(400, { success: false, error: true, message: "Contact ID and items required." }, request);
    }

    if (!Array.isArray(items) || items.length === 0) {
      return jsonResp(400, { success: false, error: true, message: "At least one item required." }, request);
    }

    const totalAmount = items.reduce((sum, item) => sum + (Number(item.amount) || 0), 0);
    if (amount !== undefined && Math.abs(amount - totalAmount) > 0.01) {
      return jsonResp(400, { success: false, error: true, message: "Invoice amount does not match items." }, request);
    }
    ...
    if (!clientResult) {
      return jsonResp(404, { success: false, error: true, message: "Contact not found." }, request);
    }
