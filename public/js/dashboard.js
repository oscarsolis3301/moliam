...
if (!session_token && document.cookie) { const match = document.cookie.match(/moliam_session=([^;]+)/); if (match) session_token=*** }
...
try {
const sessionToken=getSes...n();
if (!sessionToken) {
console.warn('No session token available for activity feed');
return;
}

const response = await fetch(`/api/activity?action=list&token=${enco...)}`, {
credentials: 'include'
});
...
const response = await fetch(`/api/activity?action=list&limit=${loadLimit}&token=${enco...)}`, {
method: 'GET',
credentials: 'include'
});
...
const response = await fetch(`/api/activity?token=${enco...t}`, {
method: 'GET',
credentials: 'include'
