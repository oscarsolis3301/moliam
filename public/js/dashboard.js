...
(async () => {
    ...
    const renderProjects = (projects) => { const grid= document.getElementById('projects-grid'); let pHtml=''; if(!projects||!projects.length)pHtml='<div class="empty-state">No active projects.</div>'; else for(const p of projects){let typeDisp = p.type ? p.type.charAt(0).toUpperCase()+p.type.slice(1)+' ' + (p.type==='Website'?':' : '') + (p.type==='Website'?'':'('+p.type+')') : ''; let statusDisp = (p.status==='active')?'Active':(p.status?(p.status.charAt(0).toUpperCase()+p.status.slice(1)):'Completed'); pHtml+='<div class="project-card"><span class="type-badge">'+typeDisp+'</span><span class="status-badge">'+statusDisp+'</span><h3>'+p.name+'</h3><div class="meta"><div>Type: '+typeDisp+'</div><div>Status: '+statusDisp+'</div></div></div>'; } grid.innerHTML=pHtml; };
    ...
    window.loadInvoicesData = async () => { try{ const r=await fetch('/api/invoices?action=list',{credentials:'include'}); return (await r.json()).data||[]; } catch(e){console.error(e);return [];}};
     ...
     // Task 14: payInvoice() - global function called from invoices-widget.js
// Pay an invoice by ID with status update confirmation to messaging system
window.payInvoice = async (invoiceId) => {
        if (!invoiceId || !String(invoiceId).trim()) {
            console.error('[payInvoice] Invalid invoice ID:', invoiceId);
            return;
         }
         
        try {
            const resp = await fetch('/api/invoices?id=' + encodeURIComponent(invoiceId) + '&action=pay',{credentials:'include'});
            const result = await resp.json();
            if (result.success) {
                Toast.success('✓ Invoice ' + result.invoice_id + ' marked as paid!');
                window.location.reload(); // Refresh invoice list
             } else {
