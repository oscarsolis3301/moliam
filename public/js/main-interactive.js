...
if (activityFeed) {
  ...
  /* ARIA live region for screen reader announcements of activity feed updates */
  let liveRegion = document.getElementById('activity-feed-live');
  if (!liveRegion) {
    liveRegion = document.createElement('div');
    liveRegion.id = 'activity-feed-live';
  ...
  setInterval(() => {
    const text = `${activities[itemIdx % activities.length]} â€” ${new Date().toLocaleTimeString()}`;
    itemIdx++;
    activityFeed.innerHTML = `<div style="font-size:13px; color:#9CA3AFF">${text}</div>` + activityFeed.innerHTML;
...
/* Keyboard handler: focus management for activity feed items */
if (activityFeed) {
  document.addEventListener('keydown', function activityFeedKeyHandler(e) {
    if (e.key === 'Tab' && activityFeed.contains(document.activeElement)) {
      // Allow natural tab flow but track if inside activity feed
    }
    if (e.key === 'Escape' && liveRegion) {
      /* Clear announcement silently */
      liveRegion.textContent = '';
    }
  });
}
