/**
 * HQ Canvas Visualization - Extracted from inline script
 * Handles the AI agent simulation canvas display
 */
(function() {
'use strict';
...
const COLORS = {
  ...
  blue:'#3B82F6', purple:'#8B5CF6', green:'#10B981', amber:'#F59E0B', red:'#EF4444', cyan:'#06B6D4', textPri:'#F9FAFB', textSec:'#9CA3AF', textDim:'#6B7280'
};
...
let bots = [];
let orbs = [];
let popover = {visible:false, id:null};
let lastTime = 0;
let hudTimer = 0;
...
function moveBot(bot, targetRoomId) {
  if(!targetRoomId || bot.room === targetRoomId && !bot.moving) return;
...
function processEvent(evt) {
  ...
  switch(evt.type) {
    ...
    case 'message_send':
      ...
      if(recipient) {
        setTimeout(()=>{ spawnOrb(bot, recipient); }, 400);
      }
      break;
    case 'error':
      bot.state = 'error';
