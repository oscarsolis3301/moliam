...
function TankGame({ accent, theme, paused }) {
  ...
  React.useEffect(() => {
    ...
    const state = {
      tank: { x: W / 2, y: H - 30, w: 36, h: 18, angle: -Math.PI / 2, speed: 0 },
      bullets: [],
      enemies: [],
      particles: [],
      keys: {},
    ...
    const loop = () => {
      ...
      state.enemies = state.enemies.filter(en => {
        ...
        // hit tank?
        const dxT = en.x - state.tank.x, dyT = en.y - state.tank.y;
  ...
  return (
    <div className="demo-body tank" tabIndex={0} onFocus={() => setRunning(true)}>
      <div className="demo-hud">
        <div className="hud-group">
          <span className="mono tiny dim">SCORE</span>
          <span className="mono num">{String(score).padStart(3, '0')}</span>
        </div>
        <div className="hud-group">
          <span className="mono tiny dim">HP</span>
          <span className="hp">
            {[0,1,2].map(i => (
              <i key={i} className={i < hp ? 'on' : 'off'} style={{background: i < hp ? accent : undefined}} />
            ))}
          </span>
        </div>
        <div className="hud-group right">
          <span className="mono tiny dim">A/D · MOVE</span>
          <span className="mono tiny dim">W/S · AIM</span>
          <span className="mono tiny dim">SPACE · FIRE</span>
        </div>
      </div>
      <canvas ref={canvasRef} style={{width: 560, height: 320, maxWidth: '100%'}} onClick={(e) => e.currentTarget.parentElement.focus()} />
    </div>
  );
}
